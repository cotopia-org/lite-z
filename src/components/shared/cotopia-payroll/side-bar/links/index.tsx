...
import { Coins, Grid, User, Wallet } from "lucide-react";
...
export default function PayrollSideBarLink() {
  ...
  const links = [
    ...
    { title: "Payments", href: "/payroll-payments", icon: <Coins /> },
              ...
              className={({ isActive }) =>
                `p-3 rounded-md flex items-center gap-x-5 text-lg font-semibold transition-all duration-50 ease-in 
                  [&_svg_path]:stroke-black 
                   ${
                     isActive
                       ? "[&_svg_path]:stroke-white bg-primary text-white border-l-4 border-black"
                       : "hover:bg-primary hover:text-white [&_svg_path]:hover:stroke-white"
                   }`
              }
              to={link.href}
            >
