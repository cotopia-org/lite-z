...
const InvisibleNode = ({ node }: { node: InvisibleNodeType }) => {
  ...
  const node_y_axis = nodeHeight / 2;
  const node_x_axis = nodeWidth / 2;
  ...
  switch (invisible_side) {
    case 'right':
      ...
      } else if (delta_y_prime - node_y_axis < 0) {
        clss += ' !bottom-[15px] right-[15px]';
      } else
        style['top'] =
          `${delta_y + (node_y_axis - VARZ.invisibleArea.margin)}px`;

      break;
    case 'left':
      clss += ' left-[15px]';
      style['top'] = `${delta_y + node_y_axis}px`;
      if (delta_y < 0) {
    ...
    case 'top':
      clss += ' top-[15px]';
      style['left'] =
        `${delta_x + (node_x_axis - VARZ.invisibleArea.margin)}px`;
      if (delta_x < 0) {
        style['left'] = `${delta_x + nodeWidth}px`;
      }
      if (delta_x_prime < -(node_x_axis - VARZ.invisibleArea.margin)) {
        style['left'] = `${delta_x - node_x_axis}px`;
      }
      break;
    case 'bottom':
      clss += ' bottom-[15px]';
      ...
      } else if (delta_x < -(node_x_axis - VARZ.invisibleArea.margin)) {
