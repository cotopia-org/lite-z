...
export const getInitMessages = createAsyncThunk(
  ...
  async ({
    ...
    let perPage = (VARZ.defaultPerPage / 2) * upper_limit;
...
export const getPrevMessages = createAsyncThunk(
  ...
  async ({
    ...
    let perPage = VARZ.defaultPerPage;
    let page = newUpperLimit;
    const isFirstPage = newUpperLimit === VARZ.pagesLimitDiff;
    if (isFirstPage) {
      perPage = (VARZ.defaultPerPage / 2) * VARZ.pagesLimitDiff;
    ...
    builder
        ...
        .addCase(
          ...
          (state, action: PayloadAction<any>) => {
            ...
            if (newMessages.length === 0) {
              ...
              updatedMessages = [
                ...prevMessages.slice(VARZ.defaultPerPage),
                ...newMessages,
              ];
            }
            return {
      ...
      builder
        ...
        .addCase(getPrevMessages.fulfilled, (state, action: any) => {
          ...
          const totalLengths = (VARZ.defaultPerPage / 2) * VARZ.pagesLimitDiff;
          const flatIndex = totalLengths - VARZ.defaultPerPage;
          //prev messages
          const prevMessages = [...(room?.messages ?? [])];
          let updatedMessages = [];
          if (isFirstPage) {
